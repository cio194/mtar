...
class HuffmanTreeNodeAdapter {
public:
  using origin_type = TreeNode;      // 原始节点类型
  using node_type = HuffmanTreeNodeAdapter; // 节点适配器类型

  // 适配器的构造函数，必须有且仅有一个指向原始节点的指针参数
  HuffmanTreeNodeAdapter(origin_type *node)
          : root_(node),
            left_(node->left_ == nullptr ? nullptr
                                         : new HuffmanTreeNodeAdapter(
                            node->left_)),
            right_(node->right_ == nullptr ? nullptr
                                           : new HuffmanTreeNodeAdapter(
                            node->right_)) {}

  // 析构函数
  ~HuffmanTreeNodeAdapter() {
    if (left_ != nullptr) {
      delete left_;
      left_ = nullptr;
    }
  ...
  std::string content() {
    ...
    if (root_->left_ == nullptr && root_->right_ == nullptr) {
      return root_->ch_ + std::string(",") + std::to_string(root_->freq_);
    } else {
      return std::to_string(root_->freq_);
    }
  }
...
public:
  ...
  SimpleTreeNodeAdapter(origin_type *root)
          : root_(root),
            ...
            right_(root->right == nullptr ? nullptr : new node_type(
                    root->right)) {}
...
private:
  ...
  public:
    ...
    Cell(int row, int col, int width, int height, vector<vector<char>> &buf,
         node_type *node = nullptr, status_type status = None)
            : node_(node), status_(status), row_(row), col_(col), buf_(&buf),
  ...
  void computeCellParams() {
    ...
    for (auto &p : colMap_) {
      ...
      contentLenMax = max(contentLenMax,
                          static_cast<int>(node->content().size()));
  ...
  void initCells() {
    cells_.assign(cellRows_, vector<Cell>(cellCols_,
                                          Cell(0, 0, cellWidth_, cellHeight_,
                                               buf_)));
  ...
  bool overlay(node_type *left, node_type *right) {
    ...
    return overlay(left->right(), right) || overlay(left, right->left()) ||
           overlay(left->right(), right->left());
  }
